...
in the middle of the alphabet, reach the end, and then start from the beginning of the alphabet.
...
array is huge (there are lots of words I don't know) so we want to be efficient here.
...
We keep track of the lower and upper bounds on the rotation point,
calling them floor_index and ceiling_index (initially we called
them "floor" and "ceiling," but because we didn't imply the type in the name
we got confused and created bugs). When floor_index and ceiling_index are directly
next to each other, we know the floor is the last item we added before starting f
rom the beginning of the dictionary, and the ceiling is the first item we added after.
...
bounded by a non-constant ll, each of our string comparisons would cost O(l)O(l),
for a total of O(l*\lg{n})O(l∗lgn) runtime.

Bonus
This function assumes that the array is rotated. If it isn't,
what index will it return? How can we fix our function to return 0 for an unrotated array?"""
